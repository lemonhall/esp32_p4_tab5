...
void LauncherView::init()
{
    ...
    _label_build_tag = std::make_unique<Label>(lv_screen_active());
    _label_build_tag->setText(std::string("LEMON ") + __DATE__ + " " + __TIME__);
    _label_build_tag->setLongMode(LV_LABEL_LONG_CLIP);
    _label_build_tag->setTextColor(lv_color_white());
    _label_build_tag->setBgColor(lv_color_black());
    _label_build_tag->setBgOpa(LV_OPA_70);
    _label_build_tag->setPadding(2, 2, 6, 6);
    _label_build_tag->setRadius(6);
    _label_build_tag->align(LV_ALIGN_TOP_LEFT, 6, 6);
    _label_build_tag->moveForeground();
    ...
    _panels.push_back(std::make_unique<PanelIrc>());
